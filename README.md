...
The verifier distinguishes a terminal Launchpad failure from an upload that never arrived. If every expected source is still invisible after 30 minutes and the Launchpad queries are otherwise reliable, reconciliation clears the pending state as retryable instead of recording a permanent failure. The next scheduled sync may therefore attempt the same unchanged release again. Temporary Launchpad API errors remain pending until visibility can be determined reliably. Once sources are visible, terminal build failures are recorded in `state/failed-<channel>.json`, and builds that remain nonterminal for more than two hours are also treated as terminal failures. Those failed releases are not retried automatically until the upstream tag or release inputs change.
