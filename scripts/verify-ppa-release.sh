...
JSON contains status=success, pending, or failed. A source that has not become
visible before PPA_SOURCE_ACCEPT_DEADLINE_SECONDS is treated as failed.
USAGE
  exit 2
}
...
for expected in manifest["sources"]:
    ...
    if publication is None:
        if age_seconds >= source_deadline:
            failure_message = (
                f"Launchpad did not expose {manifest['package']} {series} {version} "
                f"within {source_deadline}s"
            )
        else:
            any_pending = True
        source_states.append(entry)
